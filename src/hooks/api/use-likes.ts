...
function filterEvents<T extends EnrichedLikedFight | EnrichedLikedNFLGame | EnrichedLikedNBAGame | EnrichedLikedATPMatch>(
  events: T[],
  filter: Filter
): T[] {
...
export function useLikedNBAGames(filter: Filter = 'upcoming') {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Refetch the liked games when changes occur
          queryClient.invalidateQueries({ queryKey: ['liked_nba_games', user.id, filter] });
        }
      )
      .subscribe();
  ...
  const query = useQuery<EnrichedLikedNBAGame[]>({
    ...
    queryFn: async () => {
      ...
      const enrichedLikes: EnrichedLikedNBAGame[] = likedGames.map(like => ({
        ...like,
        upcoming_nba_odds: nbaOdds.find(odds => odds.game_id === like.game_id) || null
      }));
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async (gameId) => {
      ...
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: ['liked_nba_games', user.id, filter] });

      // Snapshot the previous value
      const previousLikes = queryClient.getQueryData<EnrichedLikedNBAGame[]>(['liked_nba_games', user.id, filter]);
      ...
      const newLike: EnrichedLikedNBAGame = {
        ...
        saved_at: new Date().toISOString(),
        id: Math.random(), // temporary ID
        upcoming_nba_odds: queryClient
          .getQueryData<NBAGameOdds[]>(['upcoming_nba_odds'])
          ?.find(game => game.game_id === gameId) || null,
      };

      queryClient.setQueryData<EnrichedLikedNBAGame[]>(['liked_nba_games', user.id, filter], (old = []) => {
        const newData = [newLike, ...old];
        return filterEvents(newData, filter);
      });
    ...
    onError: (err, newGame, context) => {
      if (context?.previousLikes && user) {
        queryClient.setQueryData<EnrichedLikedNBAGame[]>(['liked_nba_games', user.id, filter], context.previousLikes);
      }
    },
    onSettled: () => {
      if (user) {
...
export function useLikedATPMatches(filter: Filter = 'upcoming') {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Refetch the liked games when changes occur
          queryClient.invalidateQueries({ queryKey: ['liked_atp_games', user.id, filter] });
        }
      )
      .subscribe();
  ...
  const query = useQuery<EnrichedLikedATPMatch[]>({
    ...
    queryFn: async () => {
      ...
      const enrichedLikes: EnrichedLikedATPMatch[] = likedGames.map(like => ({
        ...like,
        upcoming_atp_odds: atpOdds.find(odds => odds.game_id === like.game_id) || null
      }));
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async (gameId) => {
      ...
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: ['liked_atp_games', user.id, filter] });

      // Snapshot the previous value
      const previousLikes = queryClient.getQueryData<EnrichedLikedATPMatch[]>(['liked_atp_games', user.id, filter]);
      ...
      const newLike: EnrichedLikedATPMatch = {
        ...
        saved_at: new Date().toISOString(),
        id: Math.random(), // temporary ID
        upcoming_atp_odds: queryClient
          .getQueryData<ATPMatchOdds[]>(['upcoming_atp_odds'])
          ?.find(game => game.game_id === gameId) || null,
      };

      queryClient.setQueryData<EnrichedLikedATPMatch[]>(['liked_atp_games', user.id, filter], (old = []) => {
        const newData = [newLike, ...old];
        return filterEvents(newData, filter);
      });
    ...
    onError: (err, newGame, context) => {
      if (context?.previousLikes && user) {
        queryClient.setQueryData<EnrichedLikedATPMatch[]>(['liked_atp_games', user.id, filter], context.previousLikes);
      }
    },
    onSettled: () => {
      if (user) {
