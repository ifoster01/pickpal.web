        ...
        <Accordion.Item key={game.game_id} value={game.game_id}>
            <Accordion.ItemTrigger _focus={{ outline: 'none', boxShadow: 'outline' }}>
                <HStack w='full' justify='space-around'>
                    <Avatar src={game.team_pic_url ?? ''} name={game.team_name?.split(' ').at(-1) ?? ''} />
                    <VStack w='full' gap={0} textAlign='center'>
                        <Text>{game.team_name?.split(' ').at(-1)}</Text>
                        <Text>{game.odds1 > 0 ? '+' : ''}{game.odds1}</Text>
                    </VStack>
                    <Text>vs</Text>
                    <VStack w='full' gap={0} textAlign='center'>
                        <Text>{game.opp_name?.split(' ').at(-1)}</Text>
                        <Text>{game.odds2 > 0 ? '+' : ''}{game.odds2}</Text>
                    </VStack>
                    <Avatar src={game.opp_pic_url ?? ''} name={game.opp_name?.split(' ').at(-1) ?? ''} />
                </HStack>
            ...
            <Accordion.ItemContent>
                <VStack w='full' px='10%'>
                    ...
                    { value !== 0 && <Text
                        ...
                        marginTop={4}
                    >
                        { odds1_diff > 0 ? game.team_name?.split(' ').at(-1) + "'s" : game.opp_name?.split(' ').at(-1) + "'s" } odds have a {betText} discrepancy from the book
                    </Text> }
                    <Scale value={value} />
                </VStack>
            </Accordion.ItemContent>
